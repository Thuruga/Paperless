...
async function loadItems() {
  try {
    toggleUIState(true);
    const snapshot = await db.collection('fb12').get();
    stateVersion.value++;
...
async function addNewItem() {
  ...
  try {
    ...
    const doc = await db.collection('fb12').doc(name).get();
    if (doc.exists) {
    ...
    await db.collection('fb12').doc(name).set({ link, category });
...
function renderItems(requestedVersion) {
  ...
  Object.entries(categories).forEach(([category, items]) => {
    ...
    items.forEach(item => column.appendChild(createItemElement(item)));
...
async function handleDelete(item) {
  ...
  try {
    ...
    await db.collection('fb12').doc(item.name).delete();
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (newName !== item.name) {
        const doc = await db.collection('fb12').doc(newName).get();
      ...
      batch.delete(db.collection('fb12').doc(item.name));
      batch.set(db.collection('fb12').doc(newName), { 
        link: newLink, 
        category: newCategory 
      });
      await batch.commit();
